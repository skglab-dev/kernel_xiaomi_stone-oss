...
extern int get_real_adapter_type(void);
				...
				const union power_supply_propval *pval)
{
	...
	type = get_real_adapter_type();
	if (type == POWER_SUPPLY_TYPE_USB_HVDCP)
		chg->therm_cur = thermal_mitigation_qc2[chg->system_temp_level];
	else
...
static int batt_get_battery_constant_current(struct batt_chg *chg,
					     int *contant_charge_current)
{
	int rc = 0;
	union power_supply_propval pval = {0, };
	if (!chg->fg_psy) {
...
static int batt_get_battery_full_design(struct batt_chg *chg,
	...
	if (rc)
		...
		if (pval.intval == 1) {
			rc = power_supply_get_property(
				chg->verify_psy, POWER_SUPPLY_PROP_MODEL_NAME,
				&pval);
			if (rc) {
			     ...
			     union power_supply_propval *pval)
{
	...
	case POWER_SUPPLY_PROP_STATUS:
		...
		if (chg->old_real_type != chg->real_type) {
			...
			schedule_delayed_work(&chg->batt_chg_work,
					      msecs_to_jiffies(100));
		}
		chg_type = get_real_adapter_type();
...
static int swchg_termination_current_adjust(struct batt_chg *chg, int temp, int type)
{
	...
	if (type == POWER_SUPPLY_TYPE_USB_PD && chg->switch_chg_ic) {
		if (temp >= chg->dt.jeita_temp_step4 &&
		    temp < chg->dt.jeita_temp_step5)
			sw_iterm = 750000; //iterm 0.15c
		if (temp >= chg->dt.jeita_temp_step5 &&
		    temp < chg->dt.jeita_temp_step6) {
...
static void sw_get_charger_type_current_limit(struct batt_chg *chg, int type)
{
		...
		case POWER_SUPPLY_TYPE_USB_HVDCP:
			chg->charge_limit_cur = 3200000;
			chg->input_limit_cur = 2000000;
			break;
	...
	if (chg->pd_cur_max != 0) {
		chg->charge_limit_cur =
			min(chg->charge_limit_cur, chg->pd_cur_max);
		chg->input_limit_cur =
			min(chg->input_limit_cur, chg->pd_cur_max);
	}
}

static int swchg_select_charging_current_limit(struct batt_chg *chg, int temp,
					       int type)
{
	int icl, ibat;
	...
	if (type != POWER_SUPPLY_TYPE_USB_PD) {
		...
		val.intval = icl;
		rc = power_supply_set_property(
			chg->sw_psy, POWER_SUPPLY_PROP_INPUT_CURRENT_LIMIT,
			&val);
		val.intval = ibat;
		rc = power_supply_set_property(
...
static int batt_parse_dt(struct batt_chg *chg)
{
	...
	rc = of_property_read_u32(node, "qcom,batt_iterm", &chg->dt.batt_iterm);
	if (rc < 0)
...
/*M17-T code for HQ-285358 by tongjiacheng at 2023/3/7 start*/
		if (vbat/1000 < (SHUTDOWN_DELAY_VOL_LOW - 100) && !chg->shutdown_delay) {
			pr_err("vbat under 3.2V, poweroff.\n");
			chg->shutdown_delay = true;
		}
/*M17-T code for HQ-285358 by tongjiacheng at 2023/3/7 end*/
	} else
		chg->shutdown_delay = false;
	...
	if (chg->last_shutdown_delay != chg->shutdown_delay) {
		...
		if (chg->shutdown_delay == true)
			strncpy(uevent_string[0] + 28, "1",
				MAX_UEVENT_LENGTH - 28);
		else
			strncpy(uevent_string[0] + 28, "0",
				MAX_UEVENT_LENGTH - 28);
		pr_err("envp[0] = %s\n", envp[0]);
...
static void batt_chg_main(struct work_struct *work)
{
	...
	if (chg->real_type) {
		...
		if (chg->ui_soc != chg->old_capacity ||
		    chg->battery_temp > 550 || chg->power_supply_count >= 6 ||
		    chg->ui_soc <= 10) {
			chg->power_supply_count = 0;
...
{
	struct batt_chg *chg =
		container_of(work, struct batt_chg, usb_type_work.work);
...
{
	struct batt_chg *chg =
		container_of(work, struct batt_chg, lower_poweroff_work.work);

	int batt_uV = 0;
	int rc;
	static int uvlo_trigger_cnt;
