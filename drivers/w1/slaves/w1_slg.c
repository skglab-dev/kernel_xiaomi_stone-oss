 ...
 * @return: 0=Success; others=failure, see Error code
 */
int slg_ecdsa_sign(unsigned char *input, int input_len, unsigned char *output,
		   int *output_len)
{
 ...
 */
int w1_slg_flag;
int get_w1_auth_result(void)
{
	return w1_slg_flag;
}
EXPORT_SYMBOL(get_w1_auth_result);
				  ...
				  struct device_attribute *attr, char *buf)
{
	...
	if (result_data.result_len > 0) {
		memcpy(buf + index, result_data.result_buf,
...

{
	...
	case SLG_MEMORY_READ:
		ret = slg_memory_read(buf[1], result_data.result_buf,
				      &result_data.result_len);
		break;

	case SLG_ECDSA_SIGN:
		ret = slg_ecdsa_sign((unsigned char *)&buf[2], buf[1],
