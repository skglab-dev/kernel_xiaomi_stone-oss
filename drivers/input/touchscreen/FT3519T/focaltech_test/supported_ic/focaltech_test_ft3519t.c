...
static int start_test_ft5452(void)
{
	...
	if (true == test_item->rawdata_test) {
		...
		if ((ret < 0) || (false == temp_result)) {
			...
			xiaomi_touch_mievent_report_str(TOUCH_EVENT_OPENTEST_FAIL, 0, "TpOpenTestFail", "focal");
#endif
#endif
#endif
		} else
	...
	if (true == test_item->rawdata_uniformity_test) {
		...
		if ((ret < 0) || (false == temp_result)) {
			test_result = false;
			fts_ftest->test_item[FTS_RAWDATA_UNIFORMITY_TEST].testresult = RESULT_NG;
		} else
			fts_ftest->test_item[FTS_RAWDATA_UNIFORMITY_TEST].testresult = RESULT_PASS;
	}
	/* scap_cb test */
	if (true == test_item->scap_cb_test) {
		...
		if ((ret < 0) || (false == temp_result)) {
			test_result = false;
			fts_ftest->test_item[FTS_SCAP_CB_TEST].testresult = RESULT_NG;
		} else
			fts_ftest->test_item[FTS_SCAP_CB_TEST].testresult = RESULT_PASS;
	}
	/* scap_rawdata test */
	if (true == test_item->scap_rawdata_test) {
		...
		if ((ret < 0) || (false == temp_result)) {
			test_result = false;
			fts_ftest->test_item[FTS_SCAP_RAWDATA_TEST].testresult = RESULT_NG ;
		} else
			fts_ftest->test_item[FTS_SCAP_RAWDATA_TEST].testresult = RESULT_PASS ;
	}
	/* short test */
	if (true == test_item->short_test) {
		...
		if ((ret < 0) || (false == temp_result)) {
			test_result = false;
			fts_ftest->test_item[FTS_SHORT_TEST].testresult = RESULT_NG;
#ifdef CONFIG_MIEV
#ifdef CONFIG_BUILD_QGKI
#ifdef CONFIG_TOUCHSCREEN_XIAOMI_TOUCHFEATURE
			xiaomi_touch_mievent_report_str(TOUCH_EVENT_SHORTTEST_FAIL, 0, "TpShortTestFail", "focal");
#endif
#endif
#endif
		} else
			fts_ftest->test_item[FTS_SHORT_TEST].testresult = RESULT_PASS;
	}
	/* panel differ test */
	if (true == test_item->panel_differ_test) {
		...
		if ((ret < 0) || (false == temp_result)) {
			test_result = false;
			fts_ftest->test_item[FTS_PANEL_DIFFER_TEST].testresult = RESULT_NG;
		} else
			fts_ftest->test_item[FTS_PANEL_DIFFER_TEST].testresult = RESULT_PASS;
	}
